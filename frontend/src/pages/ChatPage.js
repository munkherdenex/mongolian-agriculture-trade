...
const ChatPage = () => {
  ...
  useEffect(() => {
    if (!user?.id) return;
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/chat/conversations/${user.id}`)
  ...
  useEffect(() => {
    if (productId && sellerId && user?.id) {
      axios
        .post(`${process.env.REACT_APP_API_URL}/api/chat/start`, {
  ...
  const fetchMessages = useCallback(async (convId) => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/chat/messages/${convId}`);
  ...
  const sendMessage = () => {
    ...
    axios.post(`${process.env.REACT_APP_API_URL}/api/chat/send`, messageData);
  ...
  const handleSelectConversation = async (conv) => {
    ...
    try {
      await axios.patch(`${process.env.REACT_APP_API_URL}/api/chat/read`, {
